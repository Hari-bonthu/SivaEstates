...
export default function Testimonials({ lang }) {
  ...
  return (
    <section className="py-20 bg-white relative border-t border-[#E8E2DA]">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {reviews.map((rev, i) => (
            <div
              key={i}
              className="bg-[#F5F0EB] p-7 rounded-2xl border border-[#E8E2DA] flex flex-col justify-between hover:border-[#F5C6C4] transition-all"
            >
              <div className="space-y-4">
                <Quote className="w-7 h-7 text-[#C8312A] opacity-60" />
              ...
              <div className="pt-5 border-t border-[#E8E2DA] mt-5">
                <h4 className="text-sm font-bold text-[#1A1A1A] font-sans">
                  {rev.name}
                </h4>
                <p className="text-[11px] text-[#6B6860] font-sans mt-0.5">{rev.role}</p>
                <p className="text-[11px] text-[#C8312A] font-sans mt-0.5">{rev.location}</p>
              </div>
            </div>
          ))}
        </div>
